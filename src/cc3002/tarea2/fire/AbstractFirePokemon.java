...
/**
 * Abstract class that represents a fire type Pokemon.
 *
...
public abstract class AbstractFirePokemon extends AbstractPokemon implements IPokemon {
     ...
     */
    public AbstractFirePokemon(String pokedexNumber, int hp, int maxHP, IAbility selectedAbility, List<IAbility> abilityList, EnergyCounter energies) {
        super(pokedexNumber, hp, maxHP, selectedAbility, abilityList, energies);
    }

    @Override
    public void attack(ITrainer target){
        target.getActivePokemon().receiveFireAttack(this);
    }

    @Override
    public void useAbility(ITrainer target){target.getActivePokemon().receiveFireAttack(this);}
