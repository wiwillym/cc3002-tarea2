...
/**
 * Abstract class that represents a psychic type Pokemon.
 *
...
public abstract class AbstractPsychicPokemon extends AbstractPokemon implements IPokemon {
     ...
     */
    public AbstractPsychicPokemon(String pokedexNumber, int hp, int maxHP, IAbility selectedAbility, List<IAbility> abilityList, EnergyCounter energies){
        super(pokedexNumber, hp, maxHP, selectedAbility, abilityList, energies);
    }

    public void attack(ITrainer target){
        target.getActivePokemon().receivePsychicAttack(this);
    }

    @Override
    public void useAbility(ITrainer target){target.getActivePokemon().receivePsychicAttack(this);}
